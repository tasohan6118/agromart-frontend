...
const Contact = () => {
        ...
        const text = language === 'bn'
            ? {
                    title: 'যোগাযোগ রাখুন',
                    subtitle: 'নিচের যেকোনো মাধ্যম দিয়ে আমাদের সাথে যোগাযোগ করুন।',
                    formTitle: 'চলুন কথা বলি।',
                    fullName: 'পূর্ণ নাম',
                    ...
                    invalidPhone: 'অনুগ্রহ করে সঠিক বাংলাদেশি ফোন নম্বর দিন (01XXXXXXXXX)',
                    fail: 'বার্তা পাঠানো যায়নি। আবার চেষ্টা করুন।',
                }
            : {
                    title: 'Stay Connected',
                    subtitle: "We'd love to hear from you. Get in touch with us through any of the channels below.",
                    ...
                    invalidPhone: 'Please enter a valid Bangladesh phone number (01XXXXXXXXX)',
    ...
    const handleSubmit = async (e) => {
        ...
        if (!validatePhone(formData.phone)) {
            showNotification('error', text.invalidPhone);
            return;
        }
        ...
        try {
            ...
            if (response.data.success) {
                showNotification('success', text.success);
                // Reset form
                setFormData({
                    name: '',
                    email: '',
            ...
            showNotification('error', error.response?.data?.message || text.fail);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className='py-8'>
            <h1 className='text-center text-3xl font-bold mb-2'>{text.title}</h1>
            <p className='text-center font-medium mb-8'>{text.subtitle}</p>
            ...
            <div className='max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-8'>
                ...
                <div className='bg-white p-6 rounded-lg shadow-md'>
                    <form onSubmit={handleSubmit} className="flex flex-col text-sm text-slate-800">
                        <h1 className="text-3xl font-bold mb-6 text-center">{text.formTitle}</h1>
                        
                        <div className="w-full">
                            <label htmlFor="name" className="font-medium">{text.fullName}</label>
                            ...
                            <label htmlFor="email" className="font-medium mt-4">{text.email}</label>
                            <div className="flex items-center mt-2 mb-4 h-10 pl-3 border border-slate-300 rounded-full focus-within:ring-2 focus-within:ring-indigo-400 transition-all overflow-hidden">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <label htmlFor="phone" className="font-medium mt-4">{text.phone}</label>
                            <div className="flex items-center mt-2 mb-4 h-10 pl-3 border border-slate-300 rounded-full focus-within:ring-2 focus-within:ring-indigo-400 transition-all overflow-hidden">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <label htmlFor="subject" className="font-medium mt-4">{text.subject}</label>
                            <div className="flex items-center mt-2 mb-4 h-10 pl-3 pr-3 border border-slate-300 rounded-full focus-within:ring-2 focus-within:ring-indigo-400 transition-all overflow-hidden">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <select 
                                    ...
                                    onChange={handleChange}
                                    className="h-full px-2 w-full outline-none bg-transparent cursor-pointer" 
                                    required 
                                >
                                    <option value="">{text.selectCategory}</option>
                                    <option value="General Inquiry">{language === 'bn' ? 'সাধারণ জিজ্ঞাসা' : 'General Inquiry'}</option>
                                    <option value="Customer Service">{language === 'bn' ? 'গ্রাহক সেবা' : 'Customer Service'}</option>
                                    <option value="Business Development">{language === 'bn' ? 'ব্যবসা উন্নয়ন' : 'Business Development'}</option>
                                    <option value="Technical Support">{language === 'bn' ? 'প্রযুক্তিগত সহায়তা' : 'Technical Support'}</option>
                                    <option value="Marketing">{language === 'bn' ? 'মার্কেটিং' : 'Marketing'}</option>
                                    <option value="Finance">{language === 'bn' ? 'অর্থনীতি' : 'Finance'}</option>
                                    <option value="Legal">{language === 'bn' ? 'আইনি' : 'Legal'}</option>
                                </select>
                            </div>

                            <label htmlFor="message" className="font-medium mt-4">{text.message}</label>
                            <textarea 
                                name="message"
                ...
                <div className='flex flex-col gap-6'>
                    <div className='bg-white p-6 rounded-lg shadow-md'>
                        <h1 className='font-bold text-2xl mb-4 text-indigo-600'>{text.address}</h1>
                    ...
                    <div className='bg-white p-4 rounded-lg shadow-md'>
                        <h1 className='font-bold text-2xl mb-4 text-indigo-600'>{text.location}</h1>
